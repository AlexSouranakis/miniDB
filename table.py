...
class Table:
    ...
    # Updating the primary key insertion.
    # Passing an empty list as a parameter, since multiple primary keys are stored in a list.
    def __init__(self, name=None, column_names=None, column_types=None, primary_key=[]], load=None):
        ...
        # if name, columns_names and column types are not none
            ...
            # Checking if primary keys' list is empty
            if not primary_key:
                for i in primary_key: self.pk_idx.append( self.column_names.index(primary_key) )
            else:
                self.pk_idx = None
    ...
    def group_by(self, column_name):
        ...
        # Sorts based on column(s)
        idx = sorted(range(len(column_name)), key=lambda k: column_name[k])

        # If a duplicate is found, the value is stored at the first seen keym until there are no duplicates
        seen = []
        for key,val in idx: 
            if not key in seen:
                seen.append(key,val)
            if key in seen: 
                for temp_key, temp_val in idx:
                    if temp_key == key:
                        key.val += temp_key.temp_val 
                        temp_key.idx.remove(key)
        return seen
        

    def select_distinct(self, column_name):
        seen = []
        for key,val in column_name:
            if not key in seen:
                seen.append(key,val)
            else: 
                continue
